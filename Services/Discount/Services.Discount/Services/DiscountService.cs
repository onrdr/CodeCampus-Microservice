...
public class DiscountService : IDiscountService
{
    ...
    {
        var saveResponse = await _dbConnection.ExecuteAsync
    ...
    {
        var updateResponse = await _dbConnection.ExecuteAsync
            ("update discount set userid=@UserId, code=@Code, rate=@Rate where id=@Id"
                , new { discount.Id, discount.UserId, discount.Code, discount.Rate });
